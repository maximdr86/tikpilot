...
class Settings:
    ...
    def __init__(self) -> None:
        ...
        # нет LTE-модема. Одно из мест, которые ходят в интернет, поэтому
        # по умолчанию выключено: панель должна работать в сети без выхода
        # наружу. Оператор с модема читается всегда, он бесплатный.
        self.operator_lookup: bool = os.getenv("OPERATOR_LOOKUP", "0").strip() in ("1", "true", "yes", "on")

        # Смотреть ли раз в сутки, не вышла ли новая версия самой панели.
        # Выключено по умолчанию по той же причине, что и RDAP выше.
        #
        # Запрос уходит пустой: обычный GET публичной страницы выпусков,
        # без версии, без адреса установки, без чего бы то ни было о парке.
        # Разница между «схожу посмотрю» и «доложу о себе» здесь важна:
        # панель ставят на свой сервер, и телеметрия по умолчанию была бы
        # злоупотреблением доверием. Обновиться сама она всё равно не может,
        # установка это копирование файлов.
        self.update_check: bool = os.getenv("UPDATE_CHECK", "0").strip() in ("1", "true", "yes", "on")
        ...
        # Сколько процентов свободного места на диске панели считать
        # тревогой. Ноль выключает проверку. Панель следит за парком,
        # а за собственным сервером до сих пор не следила: кончившееся
        # место роняет её целиком, и узнавать об этом постфактум поздно.
        self.disk_min_free_percent: int = _int_env("DISK_MIN_FREE_PERCENT", 15)
        ...
        # --- Уведомления ------------------------------------------------------
        # Выключены, пока человек не включил: панель обещает работать в сети
        # без интернета, и незапрошенный поход наружу это нарушение обещания.
        self.notify_enabled: bool = os.getenv("NOTIFY_ENABLED", "0") not in ("0", "false", "no")
        # Сводка вместо потока событий. На парке с дребезжащими каналами это
        # единственный режим, который человек продолжает читать через месяц.
        self.notify_digest_minutes: int = _int_env("NOTIFY_DIGEST_MINUTES", 15)
        # Тихие часы по времени сервера: ночью события копятся и уходят утром.
        # Совпадающие границы означают «тихих часов нет».
        self.notify_quiet_from: int = _int_env("NOTIFY_QUIET_FROM", 23)
        self.notify_quiet_to: int = _int_env("NOTIFY_QUIET_TO", 7)
        # Пауза по паре «правило и точка»: дребезг не должен идти потоком
        self.notify_cooldown_minutes: int = _int_env("NOTIFY_COOLDOWN_MINUTES", 60)
        # Сообщать ли о возвращении в норму. Обычно да: иначе непонятно,
        # закончилось ли то, о чём написали час назад
        self.notify_resolved: bool = os.getenv("NOTIFY_RESOLVED", "1") not in ("0", "false", "no")

        # Сигнал живости наружу. Панель не может сообщить о собственной
        # смерти, поэтому делает наоборот: регулярно дёргает чужой адрес,
        # а тревогу поднимает тот, к кому сигнал перестал приходить.
        self.heartbeat_url: str = os.getenv("HEARTBEAT_URL", "").strip()
        self.heartbeat_minutes: int = _int_env("HEARTBEAT_MINUTES", 15)

        # --- Трафик на интерфейсах --------------------------------------------
        # Скорость считается из разницы счётчиков между полными обходами:
        # одна команда на все интерфейсы и средняя за интервал вместо
        # мгновенного всплеска.
        self.traffic_enabled: bool = os.getenv("TRAFFIC_ENABLED", "1") not in ("0", "false", "no")
        # По умолчанию следим только за аплинком и тем, что отмечено руками.
        # Включать сбор по всем интерфейсам стоит осознанно: на коробке с
        # тремя десятками VLAN это тридцать рядов вместо одного.
        self.traffic_all_interfaces: bool = os.getenv("TRAFFIC_ALL_INTERFACES", "0") not in ("0", "false", "no")
