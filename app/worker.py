...
def _alerts_loop() -> None:
    ...
    while not _stop.is_set():
        ...
        try:
            # Своё место на диске проверяем здесь же: кончится оно -
            # встанет вся панель, а узнать об этом постфактум поздно
            alerts.check_panel_disk()
        except Exception:  # noqa: BLE001
            log.exception("Ошибка проверки места на диске")
        try:
