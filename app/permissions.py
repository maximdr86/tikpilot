...
#:
#: Каждое право здесь обязано что-то менять: галочка, которая ничего не
#: запрещает, хуже отсутствующей, потому что создаёт уверенность, что
#: доступ ограничен. Так было с `devices.secrets`: право значилось
#: в списке, обещало закрыть пароли устройств и не проверялось нигде.
#: Тест `test_every_permission_is_actually_checked` следит, чтобы такое
#: не завелось снова.
BASE_PERMISSIONS: list[tuple[str, str, str, str]] = [
    ...
    ("terminal.use", "Устройства", "Терминал до устройства",
     ...
     "в журнал действий."),
    ("alerts.view", "Система", "Видеть пороги и срабатывания",
     "Список правил, что горит сейчас и лента событий."),
    ("alerts.manage", "Система", "Заводить и менять пороги",
     "Создание правил, правка порогов и выдержки, включение и выключение."),
