...
MIGRATIONS: dict[str, list[tuple[str, str]]] = {
    "devices": [
        ...
        ("operator_at", "TEXT"),
        # Ответила ли точка на ping с сервера панели при последней
        # неудачной проверке API, и когда спрашивали. Пусто значит
        # «не спрашивали»: проверка делается только для тех, кто не
        # ответил по API.
        ("icmp_ok", "INTEGER"),
        ("icmp_at", "TEXT"),
        # Управляется ли точка панелью: отвечает ли её API. Отдельно
        # от статуса, потому что это разные вопросы. Статус про то,
        # работает ли площадка, и на него отвечает ping; API это про
        # то, может ли панель что-то с точкой сделать.
        ("api_ok", "INTEGER"),
        ("api_seen", "TEXT"),
