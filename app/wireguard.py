...
#: Что клиент WireGuard разрешает в имени файла конфигурации. Имя файла
#: это не украшение: и `wg-quick`, и приложения на телефоне берут из него
#: название туннеля, а оно ограничено латиницей, цифрами и `_=+.-` длиной
#: до пятнадцати знаков. Файл с именем «Магазин 53.conf» приложение просто
#: откажется принимать, и человек будет думать, что дело в самой конфигурации
_WG_NAME_BAD = re.compile(r"[^A-Za-z0-9_=+.-]+")
WG_NAME_MAX = 15


def config_filename(name: str, suffix: str = ".conf") -> str:
    """
    Имя файла для скачивания конфигурации: `NV Bufet 730` → `NV-Bufet-730.conf`.

    Кириллица выкидывается, а не транслитерируется: угадывать чужую
    транслитерацию хуже, чем не угадывать вовсе, и «Магазин 53» станет
    просто `53.conf`. Если после чистки не осталось ничего, берётся `wg`:
    имя без расширения тоже должно быть непустым.

    Обрезка до пятнадцати знаков только для `.conf`, у скрипта RouterOS
    таких ограничений нет.
    """
    cleaned = _WG_NAME_BAD.sub("-", str(name or ""))
    # Подряд идущие разделители схлопываем: «metro - Gateway» иначе даёт
    # `metro---Gateway`, и человек решит, что панель сломалась
    cleaned = re.sub(r"-{2,}", "-", cleaned).strip("-._=+")
    if suffix == ".conf":
        cleaned = cleaned[:WG_NAME_MAX].strip("-._=+")
    return (cleaned or "wg") + suffix
