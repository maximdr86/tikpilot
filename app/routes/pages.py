...
#: С какого простоя точка попадает в отчёт для руководителя отдельным
#: числом. Час выбран не как «важный порог», а как граница между двумя
#: разными разговорами: до часа это перезагрузка, гроза, авария у соседа,
#: и оно проходит само. Дольше часа кто-то уже поехал или должен поехать.
LONG_OUTAGE = 3600
...
#: Потолок на список массовых сбоев. Это страховка от месяца, в котором
#: случилась беда, а не редактура: пятнадцать эпизодов за месяц надо
#: показать все. Обрезанный список порождает вопрос «а какие остальные»,
#: и ответить на него негде: массовые сбои считаются только в этом отчёте,
#: в техническом их нет. Частота вдобавок сама по себе довод: пятнадцать
#: случаев за месяц это не «пять крупных и что-то ещё», а диагноз, и
#: увидеть, что они кучкуются по утрам, можно только на полном списке.
MASS_OUTAGE_LIMIT = 30
...
async def availability_summary_page(request: Request, hours: int = 720,
    ...
    # Отдельно от кучек: сколько точек лежало долго. Само по себе число
    # «сколько точек теряли связь» ничего не решает, потому что складывает
    # несравнимое: точка, моргнувшая на две минуты, и точка, лежавшая
    # шесть часов, дают в нём одинаковый вклад. Продолжительность и есть
    # то, что отличает моргание от аварии
    long_down = [r for r in rows if r["down_seconds"] >= LONG_OUTAGE]

    # Все точки, а не пятёрка худших. Пятёрка отвечала на вопрос «где хуже
    # всего», но не отвечала на вопрос, который задают заказчику: «а что
    # было у меня». Точка, не попавшая в пятёрку, выглядела не как точка
    # без простоя, а как точка, про которую промолчали.
    #
    # Порядок по простою, сверху вниз: строки с нулём собираются в хвосте
    # и не мешают глазу, но остаются на листе как доказательство, что их
    # проверяли. Обычный парк это несколько проблемных точек и длинный
    # спокойный хвост, и такой список читается быстрее, чем кажется.
    sites = sorted(rows, key=lambda r: (-r["down_seconds"], -r["outages"],
                                        r["name"].lower()))
