...
def act_speedtest(mt: MikroTik, device: dict[str, Any], params: dict[str, Any]) -> str:
    ...
    # В строку попадает только то, что мерили. RouterOS не сообщает
    # встречное направление, если его не запрашивали, и «передача -»
    # при направлении «приём» читается как поломка теста, хотя тест
    # отработал ровно так, как просили.
    #
    # Единицы одни на все строки: столбик, где рядом стоят «104 бит/с»
    # и «61,7 Мбит/с», заставляет пересчитывать в уме на каждой строке.
    measured: list[str] = []
    if direction in ("receive", "both"):
        measured.append(f"приём {_mbit(result.get('rx'))}")
    if direction in ("transmit", "both"):
        measured.append(f"передача {_mbit(result.get('tx'))}")

    line = (f"До {target['name']} ({target['host']}): "
            f"{', '.join(measured)} Мбит/с, "
