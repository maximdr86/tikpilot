...
def test_brief_report_keeps_today_apart_from_the_period(client, router):
    ...
    assert плитка("Всего точек") == 3
    assert плитка("Сейчас недоступны") == 1, "молчащая точка не в текущем статусе"
    assert плитка("Весь период на связи") == 1
    # Четвёртое число живёт на другой оси и в эти три кучки не входит:
    # получасовой перебой в него не попал, а молчащая сейчас точка попала,
    # потому что она недоступна дольше часа
...
def test_brief_report_lists_every_site_not_just_the_worst(client, router):
    ...
    Пятёрка худших отвечала на вопрос «где хуже всего», но не отвечала
    на тот, который задаёт заказчик: «а что было у меня». Точка, не попавшая
    в пятёрку, выглядела не как точка без простоя, а как точка, про которую
    промолчали. Спокойные строки остаются на листе как доказательство, что
    их проверяли, и сортировка сводит их в хвост.
    """
    from app.database import execute, utcnow

    группа = client.post("/api/groups", data={"name": "Полная"}).json()["id"]
    имена = ["полная-тихая-1", "полная-тихая-2", "полная-тихая-3",
             "полная-тихая-4", "полная-тихая-5", "полная-шумная"]
    точки = {имя: _add_device(client, router, имя) for имя in имена}
    execute("UPDATE devices SET group_id = ? WHERE id IN (%s)"
            % ",".join("?" * len(точки)),
            (группа, *точки.values()))
    execute(
        "INSERT INTO status_events (device_id, device_name, device_host,"
        " status, reason, downtime, ts) VALUES (?,?,?,'online','',?,?)",
        (точки["полная-шумная"], "полная-шумная", "127.0.0.1", 3600, utcnow()))

    text = client.get(f"/monitoring/summary?hours=24&group_id={группа}").text
    таблица = text.split("Простой по точкам", 1)[1]

    # Шесть точек, хотя проблема была у одной: пятёрка бы их не показала
    for имя in имена:
        assert имя in таблица, имя
    # Точка с простоем стоит выше спокойных
    порядок = [таблица.index(имя) for имя in имена]
    assert порядок[-1] == min(порядок), "шумная точка не первая в списке"
    # Спокойные строки приглушены, а не выброшены
    assert "calm" in таблица


def test_brief_report_counts_how_long_sites_were_down_not_how_many_blinked(client, router):
    """
    Четвёртое число это продолжительность, а не количество.

    Раньше там стояло «сколько точек теряли связь». Оно складывало
    несравнимое: точка, моргнувшая на две минуты, и точка, лежавшая шесть
    часов, давали одинаковый вклад, и по числу нельзя было понять, тревога
    это или обычная неделя. Вдобавок оно было разностью трёх соседних
    плиток и не приносило ничего нового.
    """
    from app.database import execute, utcnow

    группа = client.post("/api/groups", data={"name": "Долгая"}).json()["id"]
    моргнула = _add_device(client, router, "моргнула-и-встала")
    лежала = _add_device(client, router, "лежала-полдня")
    execute("UPDATE devices SET group_id = ? WHERE id IN (?, ?)",
            (группа, моргнула, лежала))
    for точка, имя, секунд in ((моргнула, "моргнула-и-встала", 120),
                               (лежала, "лежала-полдня", 6 * 3600)):
        execute(
            "INSERT INTO status_events (device_id, device_name, device_host,"
            " status, reason, downtime, ts) VALUES (?,?,?,'online','',?,?)",
            (точка, имя, "127.0.0.1", секунд, utcnow()))

    text = client.get(f"/monitoring/summary?hours=24&group_id={группа}").text
    хвост = text.split("Лежали дольше часа", 1)[1]
    assert int(re.search(r">(\d+)<", хвост).group(1)) == 1, "моргание сочтено аварией"

    # Прежнее число ушло вместе с плиткой, а не переехало под новый заголовок
    assert "Перебои, сейчас на связи" not in text
